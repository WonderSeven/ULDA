...
@Classifers.register('protonet_snr')
def ProtoNetHead(query, support, support_labels, n_way, n_shot, normalize=False):
    # TODO: [2020-1-5] Set normalize=True, otherwise it will not converge
    tasks_per_batch = query.size(0)
    n_support = support.size(1)
    n_query = query.size(1)
    ...
    prototypes = torch.bmm(labels_train_transposed, support)  # [batch_size, n_way, dim]
    # Divide with the number of examples per novel category.
    prototypes = prototypes.div(
        labels_train_transposed.sum(dim=2, keepdim=True).expand_as(prototypes)
    )
    ...
    # Distance Matrix Vectorization Trick
    # TODO: [2020-1-4] anchor: query images
    # A = query.var(dim=-1, keepdim=True)  # [batch_size, n_query, 1]
    # new_query_imgs = query.unsqueeze(2)
    # new_query_imgs = new_query_imgs.repeat([1, 1, n_way, 1])  # [batch_size, n_query, n_way, dim]
    # new_query_imgs = new_query_imgs.permute(1, 0, 2, 3)
    # new_query_imgs = prototypes - new_query_imgs
    # new_query_imgs = new_query_imgs.permute(1, 0, 2, 3)
    # A_B = new_query_imgs.var(dim=-1)  # [batch_size, n_query, n_way]

    # TODO: [2020-1-4] anchor: support images
    new_prototypes = prototypes.unsqueeze(1)
    new_prototypes = new_prototypes.repeat([1, n_query, 1, 1])  # [batch_size, n_way, n_query, dim]
    A = new_prototypes.var(dim=-1)
    new_prototypes = new_prototypes.permute(2, 0, 1, 3)  # [n_way, batch_size, n_query, dim]
    new_prototypes = query - new_prototypes

    new_prototypes = new_prototypes.permute(1, 2, 0, 3)
    A_B = new_prototypes.var(dim=-1)  # [batch_size, n_query, n_way]
